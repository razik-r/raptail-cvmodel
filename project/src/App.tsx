...
function App() {
  ...
  useEffect(() => {
    const newTotal = items.reduce((sum, item) => sum + (item.payable || 0), 0);
    setTotal(newTotal);
  }, [items]);
                  ...
                  <div key={item.name} className="  border-t border-gray-300 rounded-2xl p-5  flex items-center justify-between">
                    <div className="p-2 gap-2 flex ">
                      <div className="flex size-14 bg-gray-300 rounded-[5px] border ">
                        <img
                          src={`${API_BASE_URL}/images/${item.imageUrl || 'default.png'}`}
                          alt={item.name}
                          className="size-14 object-cover rounded-[5px] border"
                          onError={(e) => {
                            (e.target as HTMLImageElement).src = `${API_BASE_URL}/images/default.png`;
                          }}
                        />
                      </div>
